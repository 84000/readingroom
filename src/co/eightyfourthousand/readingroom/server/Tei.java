...
import co.eightyfourthousand.readingroom.shared.MilestoneItem;
...
/**
 * 
 * Class that handles parsing and processing of TEI
 * 
 * @author curtis
 *
 */
public class Tei
{
	...
	public void parse(String textTei)
	{
		...
		if (biblStart >= 0)
		{
			...
			biblStr = findAndProcessUrlRefs(biblStr);
	...
	private void processGlossary(String str)
	{
		...
		// Limit glossary substitutition to the body (i.e., not the back matter)		
		int transPartCount = sectionCount + chapterCount + colophonCount;
		//for (int i = 0; i < translationParts.size(); i++)
		for (int i = 0; i < transPartCount; i++)
	...
	private void processForwardGlossInstances(TextPartData translationPart, String partId)
	{
		...
		for (int termNum = 0; termNum < textData.glossItems.size(); termNum++)
		{
			...
			while ((termBegin = oldContentLower.indexOf(term, searchPoint)) >= 0)
			{
				...
				// NEW: re-create "name" so capitalization is matched with context
				GlossItem newItem = glossItem.createDuplicate();
				newItem.name = oldContent.substring(termBegin, termEnd);
	...
	{
		// EXAMPLE:	<ref cRef="F.10.b"/>
		
		return findAndProcessRefs(data.getContent());
	}
	
	private String findAndProcessRefs(String xmlStr)
	{
		// EXAMPLE: <ref cRef="F.10.b"/>

		int refCount = 0;
		String refStr = null, refNumStr = "", linkTextStr = "";
		int refBegin, refEnd, prevRefEnd = 0, linkTextBegin, linkTextEnd = 0;
		int textBegin, textEnd;
		String newXmlStr = "";
		...
		while (true)
		{
			...
			if (refBegin >= 0 && refEnd > refBegin)
			{
				...
				// link text
				linkTextBegin = refEnd;
				linkTextEnd = xmlStr.indexOf("<", linkTextBegin);
				linkTextStr = xmlStr.substring(linkTextBegin, linkTextEnd);				

				// New content string replaces gloss item with HTML
				newXmlStr += xmlStr.substring(prevRefEnd, refBegin);
				
				if (refNumStr.startsWith("http"))
				{
					newXmlStr += "<a href=\"" + refNumStr + "\" target=\"_blank\">" + linkTextStr + "</a>";					
					prevRefEnd = linkTextEnd + "</ref>".length();
				}
				else
				{
					newXmlStr += "[" + refNumStr + "] ";
	...
	private String findAndProcessUrlRefs(String xmlStr)
	{
		// EXAMPLE: <ref target="http://www.nytimes.com">Link Text</ref>

		int refCount = 0;
		String refStr = null, urlStr = "", linkTextStr = "";
		int refBegin, refEnd, prevRefEnd = 0, linkTextBegin, linkTextEnd = 0;
		int textBegin, textEnd;
		String newXmlStr = "";
		...
		while (true)
		{
			refBegin = xmlStr.indexOf("<ref target=", prevRefEnd);
			refEnd = xmlStr.indexOf(">", refBegin) + ">".length();
			...
			if (refBegin >= 0 && refEnd > refBegin)
			{
				...
				if (textBegin >= 0)
				{
					textBegin += "\"".length();
					textEnd = refStr.indexOf("\"", textBegin);
					urlStr = refStr.substring(textBegin, textEnd);
				}
				...
				// New content string replaces gloss item with HTML
				newXmlStr += xmlStr.substring(prevRefEnd, refBegin);
				newXmlStr += "<a href=\"" + urlStr + "\" target=\"_blank\">" + linkTextStr + "</a>";					
